...
long long find(long long cum_freq) {
    ...
    while ((idx < MAX) && (bit_mask != 0)) {
        ...
        if (tIdx < MAX && cum_freq == bit[tIdx]) return tIdx;
...
##### 1D 一维
###### rmq_basic 基础区间最值查询（通常是 $O(n^2)$ 预处理或简单的线性查询）
```cpp
// 基础区间最值查询 (RMQ) - 原始暴力版
// 假设说明：所有下标均从 0 开始

const int MAX = 1e3 + 3; // 最大支持 1000 个元素
vector<int> inp;         // 原始数组
int pre[MAX][MAX];       // 存储结果：pre[i][j] 表示区间 [i, j] 的最值

// 时间复杂度: O(N^3) —— 三层循环，效率极低
// 空间复杂度: O(N^2)
void build_rmq() {
    int n = inp.size();
    for(int i = 0; i < n; ++i) {
        for(int j = i; j < n; ++j) { // 修正：原代码 j 也是从 0 开始且循环条件里写成了 ++i
            int val = -1; // 初始化为最小值
            for(int k = i; k <= j; ++k) {
                val = max(val, inp[k]);
            }
...
// RMQ - 动态规划优化版
// 原理：pre[i][j] = min(pre[i][j-1], inp[j])
...
void build_rmq() {
    int n = inp.size();
    for(int i = 0; i < n; ++i) {
        pre[i][i] = inp[i]; // 单个元素的最值就是它自己
        for(int j = i + 1; j < n; ++j) { // 修正：原代码写成了 ++i，会导致死循环
            int val = pre[i][j-1];
            if (inp[j] < val) { // 这里演示的是求最小值 (Min)
                val = inp[j];
            }
            pre[i][j] = pre[j][i] = val; // 利用对称性同时赋值
        }
    }
}
...
const int MAX = 1e5 + 5;
const int LIM = 4e5 + 5;    // 修正：线段树数组通常建议开 MAX 的 4 倍
...
// 时间复杂度: O(log n)
int query(int t, int i, int j, int l, int r) { // 修正：原代码写成了 void，应返回 int
    if (i > r || j < l) return INT_MIN; // 如果当前区间完全不在查询范围内，返回极小值
...
void update(int t, int i, int j, int p, int v) {
    ...
    int mid = (i + j) >> 1;
    if (p <= mid) update(t * 2, i, mid, p, v);
    else update(t * 2 + 1, mid + 1, j, p, v);
